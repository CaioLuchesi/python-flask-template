...
- 🎯 Entry points to execute your program using `python -m <python_flask_template>` or `$ python_flask_template` with basic CLI argument parsing.
...
---
# python_flask_template Flask Application

Awesome python_flask_template created by CaioLuchesi
...
```bash
git clone https://github.com/CaioLuchesi/python-flask-template python_flask_template
...
```bash
pip install python_flask_template
```
...
```bash
$ python -m python_flask_template
```
...
```bash
python_flask_template create-db   # run once
python_flask_template populate-db  # run once (optional)
python_flask_template add-user -u admin -p 1234  # ads a user
python_flask_template run
```
